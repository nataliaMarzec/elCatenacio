...
class Login extends React.Component {
  ...
  // login() {
  //   console.log("login", this.props.context)
  //   try {
  ...
  //       .then(this.verificarUsuario(this.props.context.usuario))
  ...
  loginResponsable(usuario) {
    try {
      fetch(`http://localhost:8383/responsable/${usuario.username}/${usuario.email}/`, {
        method: "post",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        ...
        .then(res => this.resStatusErrorResponsable(res))
        .then(this.props.context.setStateUsuario(this.props.context.usuario))
    } catch (error) {
      console.log(error);
      this.props.context.estadoInicial()
  ...
  rol(usuario) {
    if (!usuario) {
      this.loginResponsable(this.props.context.usuario)
    }
    if (usuario) {
  ...
  resStatusError = (res) => {
    ...
    if (res.status == 200) {
      console.log("esta logueado", this.props.context.usuario.rol)
    }

  }
  resStatusErrorResponsable = (res) => {
    console.log("resLogin", res)
    if (!res) {
      this.props.history.push(`/login`);
      console.log("ingrese un usuario", res)
    ...
    if (res.status == 400) {
      this.props.context.estadoInicial()
      console.log("responsable no encontrado 404", res)
    ...
    if (res.status == 200) {
      this.props.context.setStateRol("RESPONSABLE")
      this.props.context.logueado(true)
      this.props.history.push(`/`);
      console.log("esta logueado  responsable", res)
    }
  }
